...
class PLCDataComms(object):
    ...
    def get_PLC_data(self, tagList, rate):
        ...
        try:
            ...
            if rate == "Slow":
                self.comm._sock.sock.settimeout(1.5)
                self.rate = 1
            elif rate == "Medium":
                self.comm._sock.sock.settimeout(1)
                self.rate = 0.5
            else:
                self.comm._sock.sock.settimeout(0.5)
                self.rate = 0.001
    ...
    def loop_read(self, sid):
        ...
        while not self.stopThread.is_set():
            try:
                ...
                self.data = {"Custom_Status": "Success", "Values": tagValues, "DataTypes": dataTypes, "ErrorCodes": errorCodes}
